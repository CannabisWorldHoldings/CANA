...
test('Site Cortex rejects browser observations whose console or accessibility court failed',()=>{for(const key of ['consoleResult','accessibilityResult']){const payload=browserPayload();payload[key]={status:'FAIL'};const receipt=makeReceipt({kind:'BROWSER_OBSERVATION',subjectDigest:'route:/',realm:'VERIFIED_LOCAL',issuer:'browser',payload});assert.throws(()=>validateBrowserObservationReceipt(receipt),(error)=>error?.code===`BROWSER_RECEIPT_${key==='consoleResult'?'CONSOLE':'ACCESSIBILITY'}_FAILED`);}});
