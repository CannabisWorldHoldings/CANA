...
export async function issueValueReceipt({settlement,economics,evidenceAdapter,issuer='canonical-economic-settlement'}){
  ...
  const payload={receiptId:newId('value'),experimentId:settled.experimentId,settlementDigest:canonicalSettlement.receiptDigest,estimatedEffect:settled.stats,causalStatus:settled.causalStatus,revenueImpactUsd,costUsd:null,economicStatus:revenueImpactUsd===null?'UNMEASURED_OR_UNSUPPORTED':'MEASURED_WITH_CAUSAL_SUPPORT',uncertainty:settled.sufficient?'BOUNDED_BY_EXPERIMENT':'HIGH',attributedAovReceiptDigest:aovReceipt?.receiptDigest??null,createdAt:new Date().toISOString()};
