...
export function requireExperienceCandidateResult(candidate){
  assert(candidate?.manifestAfter&&candidate?.manifestAfterDigest,'experience candidate exact result required','EXPERIENCE_CANDIDATE_RESULT_REQUIRED');
  assert(digest(candidate.manifestAfter,'experience_manifest')===candidate.manifestAfterDigest,'experience candidate result digest mismatch','EXPERIENCE_CANDIDATE_RESULT_MISMATCH');
  return candidate;
}
export function createExperienceCandidate({objective,target,operations,manifestBefore=null,manifestAfter=null,proposer,evidenceReceiptDigests=[],experimentId=null,merchantId=null,tenantId=null}){
  ...
  const c={candidateId:newId('experience'),objective,target,operations,manifestBefore,manifestAfter,manifestAfterDigest:manifestAfter?digest(manifestAfter,'experience_manifest'):null,proposer,evidenceReceiptDigests,experimentId,merchantId,tenantId,status:'CANDIDATE_ONLY',blastRadius:blast,authorityRequired:merchantId&&tenantId?'OWNER_AND_MERCHANT':blast==='LOW'?'OWNER_OR_DELEGATED':'OWNER',mayExecute:false,mayPublish:false,createdAt:new Date().toISOString()};
