...
const DURABILITY_BASE = 'bf3ffc9a7a4f91f4b491b2f9d9b066bbe749c436';
const ASSESSMENT_COMMIT = 'bf3ffc9a7a4f91f4b491b2f9d9b066bbe749c436';
const GIT_BLOB_PROOF_PATHS = Object.freeze({
  'git.owner_console': 'apps/web/src/app/admin/console/page.tsx',
  'git.owner_adapter': 'apps/web/src/lib/cana-intelligence/canonical-owner-adapter.ts',
  'git.cana_intelligence_host': 'apps/web/src/lib/cana-intelligence/canonical-host.mjs',
  ...
  'git.application_experience_fabric': 'apps/web/src/lib/experience/fabric.mjs',
  ...
  'git.hyperdrive_runtime': 'apps/web/src/lib/prisma-cloudflare.ts',
  'git.artifact_portability': 'deploy/namecheap/artifact-exclusions.mjs',
...
function validateProof(proofId, proof) {
  ...
  if (proof.type === 'assessment_observation') {
    ...
    if (available) {
      ...
      const cumulative = proof.diff_mode === 'CUMULATIVE_BASE_DIFF';
      if (cumulative) assert.match(proof.base_commit, /^[0-9a-f]{40}$/, `${proofId}.base_commit`);
      else assert.equal(proof.base_commit, undefined, `${proofId}.base_commit requires cumulative mode`);
      const paths = cumulative
        ? git(['diff', '--name-only', `${proof.base_commit}..${proof.commit}`], { encoding: null })
        : git(['diff-tree', '--no-commit-id', '--name-only', '-r', proof.commit], { encoding: null });
      const patch = cumulative
        ? git(['diff', '--binary', `${proof.base_commit}..${proof.commit}`], { encoding: null })
        : git(['show', '--format=', '--binary', proof.commit], { encoding: null });
