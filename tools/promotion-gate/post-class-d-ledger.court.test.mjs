...
test('laundering, omissions, evidence drift, credentials, inclusion drift, and custody drift fail closed', () => {
  ...
  const staleAssessment = structuredClone(manifest);
  staleAssessment.proofs['git.owner_adapter'].commit = 'e0466894121a4c92f0512cfa47e649815c7a3948';
  assert.throws(() => validateProofAllowlist(staleAssessment), /git\.owner_adapter\.commit semantic drift/);
