...
| 4 | Apply committed migrations via `migrate.sh` (`prisma migrate deploy`) | PROVABLE (method) | Three committed migrations exist; `migrate.sh` requires both URL contracts and a provider/operator backup receipt. On-host proof remains owner-gated |
