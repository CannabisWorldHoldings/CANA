...
The deterministic court derives its findings from the actual desktop/mobile SVG bytes, hashes, responsive dimensions, embedded variant metadata, generation/analysis receipts, campaign genome, disclosure, rights state, copy, and performance budget. Every judge records concrete visible observations including the recognized motif, path/rectangle/circle counts, palette, minimum declared geometry opacity, responsive ratios, copy lengths, disclosure, and rights state. The deterministic fixture grammar admits opaque hex colors, numeric or percentage opacity, and translate/rotate transforms; visibility/display attributes, CSS hooks, clipping/masks, rgba/hsla colors, matrix/scale transforms, invalid opacity, and opacity below ten percent fail closed. A blank, hidden, or effectively transparent SVG carrying expected metadata therefore fails. The court refuses an unsealed caller-supplied inspection. Owner taste is explicitly `PENDING_OWNER_DECISION`, so each candidate ends at `TECHNICAL_PASS_OWNER_REVIEW_REQUIRED`, never a final visual approval. Any technical failure blocks the candidate. Regeneration is bounded to five attempts; the slice demonstrates an artifact-mismatched generic first attempt and a coherent second attempt.
